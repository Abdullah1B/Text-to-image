...
class Generator(nn.Module):
	def __init__(self,channels = 3,noise_d = 100,  text_dim = 768 , project_dim = 128, features_num = 64):
		...
		self.generator = nn.Sequential(
											nn.ConvTranspose2d(in_channels = self.project_dim + self.noise_d, 
																out_channels = self.features_num * 8,
																kernel_size = 4,
																stride=1,
																padding=0,
																bias=False),
											nn.BatchNorm2d(self.features_num * 8),
											nn.ReLU(True),

											nn.ConvTranspose2d(in_channels = self.features_num * 8, 
																out_channels = self.features_num * 4, 
																kernel_size = 4,
																stride=2,
																padding=1,
																bias=False),

											nn.BatchNorm2d(self.features_num * 4),
											nn.ReLU(True),

											nn.ConvTranspose2d(in_channels = self.features_num * 4, 
																out_channels = self.features_num * 2, 
																kernel_size = 4,
																stride=2,
																padding=1,
																bias=False),

											nn.BatchNorm2d(self.features_num * 2),
											nn.ReLU(True),

											nn.ConvTranspose2d(in_channels = self.features_num * 2, 
											...
											nn.BatchNorm2d(self.features_num),
											nn.ReLU(True),

											nn.ConvTranspose2d(in_channels = self.features_num, 
...
class Discriminator(nn.Module):
	...
	def __init__(self,channels= 3, image_size= 64, text_dim = 768, project_dim = 128, features_num = 64):
		...
		self.image_features = nn.Sequential(
								...
								nn.Conv2d(in_channels = self.features_num, 
											out_channels= self.features_num * 2, 
											kernel_size = 4, 
											stride = 2, 
											padding = 1, 
								...
								nn.Conv2d(in_channels = self.features_num * 2, 
											 out_channels= self.features_num * 4,
											 kernel_size = 4, 
											 stride = 2, 
											 padding = 1, 
								...
								nn.Conv2d(in_channels = self.features_num * 4, 
											out_channels= self.features_num * 8, 
											kernel_size = 4, 
											stride = 2, 
											padding = 1, 
		...
		self.discriminator = nn.Sequential(
								...
								nn.Conv2d(in_channels= self.features_num * 8 + self.project_dim, 
											out_channels= 1, 
											kernel_size= 4, 
											stride= 1, 
											padding= 0, 
